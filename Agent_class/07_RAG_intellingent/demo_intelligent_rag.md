...
### 修复LLM意图分析失败问题 (2025-12-07)

**问题描述：**
在切换到LLM模式后，意图识别模型一直报错：`LLM意图分析失败: Missing value for input variable "needs_retrieval": true/false, "reason": "原因说明"`

**问题原因：**
提示词模板中包含了未转义的大括号`{}`，导致LangChain将其解析为变量占位符。

**解决方案：**
1. 使用双大括号`{{}}`对提示词中的JSON格式示例进行转义
2. 增强错误处理机制，确保即使LLM响应格式异常也能回退到规则判断模式
3. 添加详细的调试日志，便于追踪问题

**验证结果：**
修复后，LLM能够正确返回JSON格式响应，系统可以正常解析并做出准确的意图判断。